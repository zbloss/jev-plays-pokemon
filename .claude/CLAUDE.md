...
### Architecture
jev-plays-pokemon is a small agent-driven Pokemon-playing harness: it consumes game state, agent instructions, and local MCP/Claude configuration, runs a decision loop through main.py, and produces the next play actions, operational logs, and issue-triage artifacts that let Jev continue the run. Like Claude Plays Pokemon, but with Jev, the repository is less a game engine than a control plane: it packages the prompts, standards, and tool boundaries that turn a language model into a persistent player with a repeatable way to act, observe, and record what happened. The repository combines several evidence views rather than one monolithic scanner. The executable view is the play loop itself: the code that starts the run, calls the agent, and returns an action.

### Key modules
- `root` — The subsystem is centered on the repository root and the Python entry-point source file main.py
