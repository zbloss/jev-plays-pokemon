...
- `src/jev_plays_pokemon/cli.py`

### Files that need care (bug-fix history first, then churn — check `get_risk` before editing)
- `src/jev_plays_pokemon/main.py` — 2 bug fixes, last fix today; 14 commits/90d
- `tests/test_decision.py` — 1 bug fix, last fix today; 11 commits/90d
- `src/jev_plays_pokemon/decision.py` — 1 bug fix, last fix today; 9 commits/90d
- `src/jev_plays_pokemon/stream_surface.py` — 5 commits/90d
- `tests/test_stream_surface.py` — 5 commits/90d

### Code health
Three co-equal signals: code health 8.23/10 avg (Good), hotspot health 6.53/10 (stable), worst `src/jev_plays_pokemon/main.py` at 3.76/10 · maintainability 9.16/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `src/jev_plays_pokemon/main.py` — change entropy — impact −1.5
- `src/jev_plays_pokemon/decision.py` — change entropy — impact −1.4
