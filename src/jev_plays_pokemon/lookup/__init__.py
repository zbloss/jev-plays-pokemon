...
Covers the three RAM-exposed ID spaces that each have a single, fixed,
documented enum in `pret/pokered`'s disassembly constants: items
(`item_name`), species (`species_name`), and map locations (`map_name`).
...
__all__ = ["item_name", "map_name", "species_name"]
